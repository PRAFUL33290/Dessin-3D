# Dessin-3D

# 🎨➡️🧊 Atelier : Mon dessin devient 3D

## Structure actuelle

- **`index.html`** → Page principale de l’atelier (pour les enfants)
  - Idées de dessins
  - Étapes claires
  - **Fiche création** imprimable (à remplir à la main ou à imprimer)

- **`3D.html`** → Visionneuse 3D pour les modèles GLB générés avec Meshy AI

### Utilisation

```bash
# Lancer localement
python3 -m http.server

# Puis ouvrir :
# http://localhost:8000          → Atelier + fiche
# http://localhost:8000/3D.html  → Visionneuse 3D
```

### Pour afficher un modèle 3D

**Via URL :**
`3D.html?model=ton-modele.glb&name=Nom%20de%20la%20création`

Exemple :
- `3D.html?model=link.glb&name=Super%20Link`

**Test local :**
Ouvre `3D.html` et clique sur « Charger un .glb local » (idéal pour tester avant de mettre en ligne).

Le modèle final doit être hébergé (GitHub Pages, etc.) pour être visible publiquement.

---

# 🎨➡️🧊 Atelier : Mon dessin devient 3D

Le contenu détaillé (idées, missions, etc.) est maintenant intégré directement dans la page `index.html`.

Tu peux aussi garder cette liste pour imprimer des affiches d’inspiration.

---

## Idées de personnages

- Super-héros
- Princesse de l’espace
- Pirate du futur
- Ninja animal
- Chevalier dragon
- Robot gentil
- Magicien des bonbons
- Explorateur des océans
- Extraterrestre rigolo
- Gardien d’un trésor

---

## Idées de créatures

- Chat ailé
- Chien dragon
- Panda ninja
- Requin licorne
- Crocodile arc-en-ciel
- Dinosaure du futur
- Lion de feu
- Serpent électrique
- Lapin géant
- Monstre gentil

---

## Idées d’objets magiques

- Baguette magique
- Épée légendaire
- Montre qui arrête le temps
- Sac sans fond
- Téléphone du futur
- Couronne magique
- Chaussures volantes
- Clé qui ouvre tous les mondes
- Casque de super-pouvoirs
- Lampe magique

---

## Idées de véhicules

- Voiture volante
- Moto fusée
- Skate volant
- Bateau sous-marin
- Train des nuages
- Camion à bonbons
- Fusée familiale
- Kart futuriste
- Hélicoptère animal
- Montgolfière magique

---

## Idées gourmandes

- Pizza aventurière
- Donut robot
- Glace magicienne
- Hamburger géant
- Cupcake dragon
- Bonbon super-héros
- Sucette géante
- Gâteau château
- Pastèque extraterrestre
- Fraise volante

---

## Idées de lieux

- Maison dans les nuages
- Château sous la mer
- École du futur
- Ville de robots
- Cabane secrète
- Île aux dragons
- Volcan de glace
- Parc d’attractions géant
- Stade futuriste
- Planète imaginaire

---

## Défi bonus

Tu peux mélanger plusieurs idées.

Exemples :

- Chat + fusée = un chat astronaute
- Pizza + robot = une pizza mécanique
- Dragon + bonbon = un dragon sucré
- Ninja + panda = un panda ninja
- Voiture + ailes = une voiture volante
- Château + mer = un château sous-marin

---

## Étapes de l’activité

1. Choisis ton idée.
2. Dessine-la sur une feuille.
3. Ajoute des couleurs.
4. Donne un nom à ta création.
5. Explique son pouvoir ou son utilité.
6. L’animateur prendra ton dessin en photo.
7. Ton dessin sera transformé en création 3D avec Meshy AI.

---

## Fiche création

### Nom de ma création :

...

### C’est quoi ?

...

### Son pouvoir ou son utilité :

...

### Ses couleurs :

...

### Son monde ou son histoire :

...

---

## Nom de l’exposition

# 🌟 Le Musée des Créations du Futur

Chaque enfant pourra découvrir :

- son dessin original
- sa version en 3D
- les créations des autres enfants

---

## Petits prix possibles

- Création la plus drôle
- Création la plus magique
- Création la plus originale
- Création la plus futuriste
- Coup de cœur des animateurs# 🎨➡️🧊 Atelier : Mon dessin devient 3D

## Objectif

Aujourd’hui, tu vas inventer et dessiner quelque chose que tu aimes ou que tu imagines.

Ensuite, ton dessin sera transformé en **création 3D** grâce à l’intelligence artificielle.

---

## Mission des enfants

Dessine au choix :

- un personnage
- un animal
- un objet magique
- un véhicule
- une créature fantastique
- une maison
- une planète
- une nourriture vivante
- un héros
- quelque chose que tu adores

---

## Idées de personnages

- Super-héros
- Princesse de l’espace
- Pirate du futur
- Ninja animal
- Chevalier dragon
- Robot gentil
- Magicien des bonbons
- Explorateur des océans
- Extraterrestre rigolo
- Gardien d’un trésor

---

## Idées de créatures

- Chat ailé
- Chien dragon
- Panda ninja
- Requin licorne
- Crocodile arc-en-ciel
- Dinosaure du futur
- Lion de feu
- Serpent électrique
- Lapin géant
- Monstre gentil

---

## Idées d’objets magiques

- Baguette magique
- Épée légendaire
- Montre qui arrête le temps
- Sac sans fond
- Téléphone du futur
- Couronne magique
- Chaussures volantes
- Clé qui ouvre tous les mondes
- Casque de super-pouvoirs
- Lampe magique

---

## Idées de véhicules

- Voiture volante
- Moto fusée
- Skate volant
- Bateau sous-marin
- Train des nuages
- Camion à bonbons
- Fusée familiale
- Kart futuriste
- Hélicoptère animal
- Montgolfière magique

---

## Idées gourmandes

- Pizza aventurière
- Donut robot
- Glace magicienne
- Hamburger géant
- Cupcake dragon
- Bonbon super-héros
- Sucette géante
- Gâteau château
- Pastèque extraterrestre
- Fraise volante

---

## Idées de lieux

- Maison dans les nuages
- Château sous la mer
- École du futur
- Ville de robots
- Cabane secrète
- Île aux dragons
- Volcan de glace
- Parc d’attractions géant
- Stade futuriste
- Planète imaginaire

---

## Défi bonus

Tu peux mélanger plusieurs idées.

Exemples :

- Chat + fusée = un chat astronaute
- Pizza + robot = une pizza mécanique
- Dragon + bonbon = un dragon sucré
- Ninja + panda = un panda ninja
- Voiture + ailes = une voiture volante
- Château + mer = un château sous-marin

---

## Étapes de l’activité

1. Choisis ton idée.
2. Dessine-la sur une feuille.
3. Ajoute des couleurs.
4. Donne un nom à ta création.
5. Explique son pouvoir ou son utilité.
6. L’animateur prendra ton dessin en photo.
7. Ton dessin sera transformé en création 3D avec Meshy AI.

---

## Fiche création

### Nom de ma création :

...

### C’est quoi ?

...

### Son pouvoir ou son utilité :

...

### Ses couleurs :

...

### Son monde ou son histoire :

...

---

## Nom de l’exposition

# 🌟 Le Musée des Créations du Futur

Chaque enfant pourra découvrir :

- son dessin original
- sa version en 3D
- les créations des autres enfants

---

## Petits prix possibles

- Création la plus drôle
- Création la plus magique
- Création la plus originale
- Création la plus futuriste
- Coup de cœur des animateurs
