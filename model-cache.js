(function () {
  ...
  // Ordinateur : tous les modèles restent montés et interactifs dans la
  // galerie. Téléphone/tablette : 3D.html conserve son chargement progressif
  // et libère les modèles qui sortent de la zone visible.
  const isAppleTablet = /iPad/.test(navigator.userAgent) ||
    (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  const isPhoneOrTablet = isAppleTablet ||
    /iPhone|iPod|Android|Mobile|Tablet|Kindle|Silk\//i.test(navigator.userAgent);
  const isMuseumPage = /(?:^|\/)3d\.html$/i.test(window.location.pathname);
  const keepAllGalleryModelsOnDesktop = isMuseumPage && !isPhoneOrTablet;

  function keepDesktopGalleryModelsMounted() {
    if (!keepAllGalleryModelsOnDesktop || !('IntersectionObserver' in window)) return;

    const NativeIntersectionObserver = window.IntersectionObserver;
    if (NativeIntersectionObserver.__dessin3dDesktopPersistent) return;
    NativeIntersectionObserver.__dessin3dDesktopPersistent = true;
    ...
    function DesktopPersistentObserver(callback, options) {
      return new NativeIntersectionObserver((entries, observer) => {
        // 3D.html charge un modèle quand isIntersecting est true et le détruit
        // quand il devient false. Sur ordinateur, chaque carte reste donc
        // volontairement « visible » pour garder tous les modèles affichés.
        const persistentEntries = entries.map((entry) => {
          if (!isInlineModel(entry.target)) return entry;
          return { target: entry.target, isIntersecting: true };
        });
        callback(persistentEntries, observer);
      }, options);
    }

    DesktopPersistentObserver.prototype = NativeIntersectionObserver.prototype;
    Object.setPrototypeOf(DesktopPersistentObserver, NativeIntersectionObserver);
    window.IntersectionObserver = DesktopPersistentObserver;
  }

  keepDesktopGalleryModelsMounted();
